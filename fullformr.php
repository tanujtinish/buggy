...
<?php
	...
	$roff = $_SESSION["ID"];
	$sql2 = "SELECT * FROM employee WHERE ID ='$roff'";
    $result2 = $conn->query($sql2);
	while($row2 = mysqli_fetch_array($result2)){

		$rName = $row2["Name"];
		$rID = $row2["ID"];
		$rDesignation  = $row2["Designation"];
		$rDepartment  = $row2["Department"];
	}
	
	echo "<!DOCTYPE html>
<html lang=\"en\">
   <title>Reviewing Officer Score</title>
   <head>
      <script >
      var total0;
      var total1;
	...
	function confirmsubmission() {
					if (confirm(\"Do you really want to submit!\") == true) {
						var element = document.getElementById(\"fullformr\"); 
						element.setAttribute(\"action\", \"submitformr.php\");
					} else {
						var element = document.getElementById(\"fullformr\"); 
						element.setAttribute(\"action\", \"reviewing.php\");
					}
				}
  ...
  <form id=\"fullformr\" class=\"form-validate form-horizontal\" method=\"post\" action=\"afterlogin.php\">
  <div class=\"form-group \">
      ...
      <form id=\"formpg114\" class=\"form-validate form-horizontal\">
        ...
        <div class=\"col-lg-9\">
        <input name=\"ndRA\" class=\"form-control \" type=\"text\" value=\"$rName --> $rDesignation, $rDepartment \" readonly/>
        </div>
        </div>
        <br><br>
        <div class=\"form-group \">
